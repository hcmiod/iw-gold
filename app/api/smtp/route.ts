...
export async function GET(req: NextRequest) {
  ...
  const accounts = await db.query.iwgSmtpAccounts.findMany({
    where: (a, { eq }) => eq(a.userId, auth.userId),
...
export async function POST(req: NextRequest) {
  ...
  const existing = await db.query.iwgSmtpAccounts.findMany({
    where: (a, { eq }) => eq(a.userId, auth.userId),
  });
  const [account] = await db.insert(iwgSmtpAccounts).values({
    userId: auth.userId,
    label: body.label || `Account ${existing.length + 1}`,
    host: body.host || "smtp.gmail.com",
    port: body.port || 587,
    secure: body.secure || false,
    username: body.username,
    password: body.password,
    replyTo: body.replyTo || null,      // save reply-to address
    dailyLimit: body.dailyLimit || 500,
    throttleSeconds: body.throttleSeconds || 5,
  }).returning();
