...
export async function POST(req: NextRequest) {
  ...
  if (action === "login") {
    const user = await db.query.iwgUsers.findFirst({ where: (u, { eq }) => eq(u.email, body.email?.toLowerCase()) });
  ...
  const existing = await db.query.iwgUsers.findFirst({ where: (u, { eq }) => eq(u.email, body.email?.toLowerCase()) });
  if (existing) return NextResponse.json({ error: "Email already registered" }, { status: 409 });
  const hash = await bcrypt.hash(body.password, 12);
  const [user] = await db.insert(iwgUsers).values({ email: body.email.toLowerCase(), name: body.name, passwordHash: hash }).returning();
  return NextResponse.json({ token: signToken(user.id), user: { id: user.id, name: user.name, email: user.email } });
}
