...
export async function GET(req: NextRequest) {
  ...
  const accounts = await db.query.smtpAccounts.findMany({ where: (a, { eq }) => eq(a.userId, auth.userId), orderBy: (a, { asc }) => [asc(a.createdAt)] });
...
export async function POST(req: NextRequest) {
  ...
  const existing = await db.query.smtpAccounts.findMany({ where: (a, { eq }) => eq(a.userId, auth.userId) });
